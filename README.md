...
4. `run_onchange_after_20-brew-bundle.sh.tmpl`: runs `brew bundle --file="$HOME/.config/Brewfile"`. Re-runs automatically whenever `Brewfile` changes (chezmoi embeds a hash of it in the script). A single failing formula/cask does not abort the run. `~/.config/Brewfile` was already written in step 2 above by the time this runs, on every install, regardless of whether the source directory is `~/.local/share/chezmoi` or anywhere else, there is no fresh-install gap here. The script's own `if [[ ! -f "$BREWFILE" ]]` guard is defensive, not something that's expected to trigger.
   ...
   Set `DOTFILES_SKIP_CASKS=1` to install formulae only and skip every `cask` entry:

   ```sh
   DOTFILES_SKIP_CASKS=1 chezmoi apply
   ```

   The skip list is derived from the Brewfile at runtime (via `HOMEBREW_BUNDLE_CASK_SKIP`), so there is nothing to keep in sync as casks come and go. This exists for disk- and bandwidth-constrained throwaway machines, chiefly VM smoke tests, where 45 GUI casks are tens of GB of download that prove nothing `brew bundle check` hasn't already verified on a real machine. Don't use it on a machine you actually intend to work on. It's an environment variable rather than a prompt on purpose: this script re-runs on every Brewfile change, and a prompt would block unattended applies.

   **If `brew bundle` looks hung at a bare `Fetching ...` line**, it almost certainly isn't. Homebrew only renders its live download display when stdout is a TTY, so piping the run through `| tee` silences it for the entire parallel fetch. Use `script -q out.log ./bootstrap.sh` instead, which keeps a pty and still writes the log. To confirm progress from outside a VM, watch the disk image grow: `du -m ~/.tart/vms/<vm>/disk.img`.
5. `run_onchange_after_30-dev-tools.sh`: installs/upgrades `uv`, Python 3.10–3.14, global `uv tool` CLIs (`ruff`, `mypy`, `pytest`, `pre-commit`, `commitizen`, `cookiecutter`, `marimo`), and Rust + `rust-analyzer`. Re-runs when this script's content changes (e.g. you add a tool to it).
