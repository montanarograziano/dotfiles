...
- **Agent settings**, `~/.pi/agent/settings.json`, source `home/dot_pi/agent/settings.json`. Pi always resolves this from a fixed `~/.pi/agent/` path; it does not consult `$XDG_CONFIG_HOME`. It's a plain (non-templated) chezmoi-managed file, chezmoi writes a real file here, not a symlink.
